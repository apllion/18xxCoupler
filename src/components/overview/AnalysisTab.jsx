...
import { corpPrice, priceAt, moveDividend, moveUp, isSoldOut } from '../../engine/stockMarket.js'
...
export default function AnalysisTab() {
  ...
  return (
    <div className={m
      ...
      {/* ================================================================ */}
      {/* ENDGAME CALC — crank ORs with fixed revenues until bank breaks  */}
      {/* ================================================================ */}
      {view === 'endgame' && <EndgameCalc game={game} fmt={fmt} m={m} />}
...
// =========================================================================
// ENDGAME CALCULATOR — crank frozen-map ORs until bank breaks
// =========================================================================
function EndgameCalc({ game, fmt, m }) {
  const corps = game.corporations.filter(c => c.floated)
  const divMovement = game.title.dividendMovement || 'standard'

  // Editable revenues per corp — pre-filled from last known
  const initRevenues = () => {
    const revs = {}
    for (const c of corps) {
      let lastRev = 0
  ...
  function simulate() {
    ...
    // Clone corp cash
    const corpCash = {}
    corps.forEach(c => { corpCash[c.sym] = c.cash })
    ...
    for (let or = 1; or <= maxRounds && bank > 0; or++) {
      ...
      // Each corp operates in price order
      ...
      for (const c of ordered) {
        ...
        // Pay dividend: bank pays shareholders
        let totalPaid = 0
        for (const p of game.players) {
          const pct = playerSharePercent(p, c.sym)
          if (pct > 0) {
      ...
      // Sold-out bumps (simplified: check if IPO+market = 0)
      for (const c of corps) {
  ...
  return (
    <>
      <Panel m={m} title="Endgame Calculator">
      ...
      {result && (
        <>
          <Panel m={m} title={`Result: ${result.ors.length} ORs to bank break`}>
            {/* Per-OR summary table */}
            <div className="overflow-x-auto">
              <table className={m ? 'text-[10px] w-full' : 'text-[10px] w-full'}>
                <thead>
                  ...
                  {result.ors.map(or => (
                    <tr key={or.or} className={or.bank <= 0 ? 'text-red-400' : m ? 'text-blue-300' : 'text-broker-text'}>
                      ...
                      {corps.map(c => {
                        ...
                        return <td key={c.sym} className="text-right px-1">
                          {cd ? `${fmt(cd.priceAfter)}` : '—'}
                        </td>
                      })}
            ...
            <div className="space-y-1">
              ...
              {game.players
                .map(p => {
                  ...
                  // End state: cash from sim + share value at final prices + privates
